...
int main(int argc, char* argv[])
{
  int nrows, ncols;
  double *aa;//matrix 
  //example: element b[8] is matrix row 2 col 0 in a 4x4 matrix. 
  //example: element b[8] is matrix row 2 col 2 in a 3x3 matrix(the last element). 
  ...
  if (argc > 1) {//if more than one processor
    ...
    if (myid == master) {// Master Code goes here
         ...
         */
         for(int geo=0; geo<nrows*ncols; geo++){
            printf("\n\nc%d:%f\n\n" , geo , c[geo]);
         }
        ///////////////////////////////////F
    }
     ...
     //MPI_Bcast(data , count  , datatype , root , communicator?)
      MPI_Bcast(b, ncols*nrows, MPI_DOUBLE, master, MPI_COMM_WORLD);//I'm ready to receive!
